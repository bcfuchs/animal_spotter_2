!  function() {
    var doRss_1 = function(){
	var url = "http://feeds.bbci.co.uk/news/science_and_environment/rss.xml";
	$('#rss-feeds').rss(url, {
    ...
    var doRss = function() {
	var url = "https://feeds.bbci.co.uk/news/science_and_environment/rss.xml";
	$("#rss-feeds").rss(
	    url,
	    {
...
//		host: 'feedrapp.info',
		...
		// valid values: any string
		layoutTemplate: "<div class='feed-container list-group'>{entries}</div>",
		...
		entryTemplate: '<a class="rss-url" href="{url}"><div class="list-group-item"><div class="rss-title">{title}</div></a><div class="rss-body">{shortBodyPlain} {teaserImage}</div></div>',
		...
		onData: function(){ console.log("onData")}
	    },

	    // callback function
	    // called after feeds are successfully loaded and after animations are done
	    function callback() { console.log('animation')}
	)
